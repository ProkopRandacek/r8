...
void d2Group(vec3 pos, int i) {
	vec4  aC = mix(sC(gA(i))      , d2GroupsC[gA(i) - maxShapeNum], step(maxShapeNum-1, gA(i)));
	float aD = mix(d2Shapes[gA(i)], d2GroupsD[gA(i) - maxShapeNum], step(maxShapeNum-1, gA(i)));
	vec4  bC = mix(sC(gB(i))      , d2GroupsC[gB(i) - maxShapeNum], step(maxShapeNum-1, gB(i)));
	float bD = mix(d2Shapes[gB(i)], d2GroupsD[gB(i) - maxShapeNum], step(maxShapeNum-1, gB(i)));
...
void d2GroupD(vec3 pos, int i) {
	...
	d2GroupsD[i] = CombineD(
			mix(d2Shapes[gA(i)], d2GroupsD[gA(i) - maxShapeNum], step(maxShapeNum, gA(i))),
			mix(d2Shapes[gB(i)], d2GroupsD[gB(i) - maxShapeNum], step(maxShapeNum, gB(i))),
			gOp(i), gK(i));
}
